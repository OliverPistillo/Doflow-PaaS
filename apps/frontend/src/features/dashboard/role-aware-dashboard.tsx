...
function RoleDashboard({ user, collaboratorView = false }: { user: DoflowIdentityUser; collaboratorView?: boolean }) {
  ...
  const projectProgress = totalPhases ? Math.round(completedPhases / totalPhases * 100) : 0
  ...
  return <main className="@container/main mx-auto w-full max-w-none space-y-6 p-4 md:p-6">
    ...
    {technical && <Card><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle className="flex items-center gap-2"><Target className="size-5 text-primary" />Progetti e consegne</CardTitle><CardDescription>Avanzamento reale delle fasi collegate alle attività.</CardDescription></div><Badge variant="secondary" className="text-sm tabular-nums">{projectProgress}%</Badge></div></CardHeader><CardContent className="space-y-4"><div className="rounded-lg border bg-muted/20 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><span className="text-sm font-medium">Avanzamento complessivo</span><span className="text-xs text-muted-foreground">{completedPhases} di {totalPhases} fasi concluse</span></div><Progress className="mt-3" value={projectProgress} /></div><div className="grid gap-4 md:grid-cols-2">{projects.slice(0, 6).map((project) => { const complete = project.phases.filter((phase) => phase.status === "completed").length; const progress = project.phases.length ? Math.round(complete / project.phases.length * 100) : 0; return <Link key={project.id} href={`/dashboard/progetti/${project.id}`} className="rounded-lg border p-4 transition-colors hover:bg-muted/50"><div className="flex items-center justify-between gap-2"><span className="font-medium">{project.name}</span><Badge variant="secondary">{progress}%</Badge></div><Progress className="mt-3" value={progress} /><p className="mt-2 text-xs text-muted-foreground">Scadenza {project.dueDate || "non definita"}</p></Link> })}{!projects.length && <p className="text-sm text-muted-foreground">Nessun progetto assegnato nel perimetro autorizzato.</p>}</div></CardContent></Card>}
