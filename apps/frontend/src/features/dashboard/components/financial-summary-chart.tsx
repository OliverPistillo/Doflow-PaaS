...
export function FinancialSummaryChart({ metrics, activeMetric }: { metrics: OverviewMetric[]; activeMetric: FinancialMetric }) {
  ...
  const data = revenue.series.map((point, index) => ({ month: point.month, revenue: point.value, expenses: expenses.series[index]?.value ?? 0, profit: profit.series[index]?.value ?? 0 }))
  const hasNegativeProfit = data.some((point) => point.profit < 0); const currentSeries: FinancialMetric[] = ["revenue", "expenses", "profit"]
  return <div className="min-w-0 overflow-x-auto"><ChartContainer config={config} className="h-[235px] w-full min-w-[520px] sm:min-w-0 @md/main:h-[255px]" aria-label={`Andamento economico, metrica evidenziata ${config[activeMetric].label}`}><AreaChart data={data} margin={{ top: 8, right: 8, left: 4, bottom: 12 }}><defs>{currentSeries.map((metric) => <linearGradient key={metric} id={`fill-economic-${metric}`} x1="0" x2="0" y1="0" y2="1"><stop offset="5%" stopColor={`var(--color-${metric})`} stopOpacity={metric === activeMetric ? .18 : .05}/><stop offset="95%" stopColor={`var(--color-${metric})`} stopOpacity={0}/></linearGradient>)}</defs><CartesianGrid vertical={false}/><XAxis dataKey="month" tickLine={false} axisLine={false} tickMargin={10} interval="preserveStartEnd"/><YAxis tickLine={false} axisLine={false} tickMargin={6} width={52} tickFormatter={formatCompactCurrency}/>{activeMetric === "profit" && hasNegativeProfit && <ReferenceLine y={0} stroke="var(--border)"/>}<ChartTooltip content={<ChartTooltipContent indicator="dot" labelFormatter={(value) => `Mese: ${value}`} formatter={(value, name) => <div className="flex w-full justify-between gap-4"><span>{config[name as keyof typeof config].label}</span><span className="tabular-nums">{formatCurrencySuffix(Number(value))}</span></div>}/>}/>{currentSeries.map((metric) => <Area key={metric} dataKey={metric} type="monotone" stroke={`var(--color-${metric})`} fill={`url(#fill-economic-${metric})`} strokeWidth={metric === activeMetric ? 3 : 1.6} strokeOpacity={metric === activeMetric ? 1 : .55} dot={{ r: metric === activeMetric ? 2.5 : 1.5 }} activeDot={{ r: 4 }}/>) }</AreaChart></ChartContainer></div>
}
