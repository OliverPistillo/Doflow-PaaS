...
export function ClientQuickSheet({
  clientId,
  open,
  onOpenChange,
}: {
  ...
  const openActivities = activities.filter(
    (item) => !["Completata", "Annullata"].includes(item.status),
  );
  const nextActivity = [...openActivities].sort((left, right) =>
    (left.dueAt || left.dueDate).localeCompare(right.dueAt || right.dueDate),
  )[0];
  const project = projects.find(
    (item) => item.clientId === customer.id && !item.archivedAt,
  );
  const order = store.orders.find(
    (item) => item.customerId === customer.id && !item.archivedAt,
  );
  const contract = store.contracts.find(
    (item) =>
      item.orderId === order?.id && !item.archivedAt && !item.replacedById,
  );
  const renewal = store.renewals.find(
    (item) => item.customerId === customer.id && !item.archivedAt,
  );
  const financial =
    order && identity.hasCapability("canViewAdministration")
      ? orderFinancialsFromServer(order)
      : undefined;
  const documents = (customer.documents ?? []).filter(
    (item) => !item.archivedAt,
  );
  const communication = [...(customer.communications ?? [])]
    .filter((item) => !item.archivedAt)
    .sort((a, b) => b.occurredAt.localeCompare(a.occurredAt))[0];
  const timeline = store.timelineEvents
      ...
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent
          onInteractOutside={(event) => {
            if ((event.target as HTMLElement)?.closest("[data-flow-coachmark]"))
              event.preventDefault();
          }}
          showCloseButton={false}
          className="flex h-dvh w-full flex-col overflow-hidden p-0 sm:max-w-[520px]"
        >
          <SheetHeader className="shrink-0 border-b px-5 py-4 text-left">
            <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto_2.25rem_2.25rem] items-start gap-3">
              <div className="flex min-w-0 gap-3">
                <CustomerLogo customer={customer} className="size-11" />
              ...
              <Badge
                variant="secondary"
                className="max-w-32 whitespace-normal text-center"
              >
                {customer.status}
              </Badge>
              {canEditLogo && (
                <Button
                  size="icon-sm"
                  variant="ghost"
                  aria-label="Modifica logo cliente"
            ...
            <p className="text-xs text-muted-foreground">
              Responsabile: {customer.profile.owner}
            </p>
          </SheetHeader>
          <ScrollArea className="min-h-0 flex-1">
            <div className="space-y-4 px-5 py-4">
              ...
              <Card data-flow-tour="flow-client-quick-summary">
                ...
                <CardContent className="grid gap-3 text-sm sm:grid-cols-2">
                  <Value label="Origine" value={customer.profile.source} />
                  <Value
                    label="Ordine"
                    value={order?.code ?? "Non collegato"}
                  />
                  <div>
                    <p className="text-xs text-muted-foreground">Contratto</p>
                    <DocumentStatusBadge
                      status={contract?.status ?? "Da generare"}
                    />
                  </div>
                  <Value
                    label="Progetto"
                  ...
                  <Value
                    ...
                    value={
                      nextActivity
                        ? dateTime.format(
                            new Date(
                              nextActivity.dueAt || nextActivity.dueDate,
                            ),
                          )
                        : "Nessuna"
                    }
                  ...
                  <Value
                    ...
                    value={
                      renewal
                        ? `${renewal.status} · ${dateTime.format(new Date(renewal.nextDueAt))}`
                        : "Non previsto"
                    }
                  />
                  <Value
                    ...
                    value={
                      communication
                        ? `${communication.channel} · ${dateTime.format(new Date(communication.occurredAt))}`
                        : "Nessuna"
                    }
                  />
                  {financial && (
                    <>
                      <Value
                        ...
                        value={money.format(financial.netCollected)}
                      />
                      <Value
                        label="Residuo"
                        value={money.format(financial.residual)}
                      />
                    </>
                  )}
                </CardContent>
              ...
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-base">Documenti critici</CardTitle>
                </CardHeader>
                <CardContent className="space-y-2 text-sm">
                  <div className="flex flex-wrap gap-2">
                    <DocumentStatusBadge
                      status={contract?.status ?? "Da firmare"}
                    />
                  ...
                  <p>
                    {criticalDocuments.length
                      ? `${criticalDocuments.length} documenti ancora da completare`
                      : "Nessun documento bloccante registrato"}
                  </p>
                  <div className="flex flex-wrap gap-2">
                    <Button asChild size="sm" variant="outline">
                      <Link
                        href={`/dashboard/clienti/${customer.id}?tab=documents`}
                      >
                        <FileSignature />
                        Documenti
                      </Link>
              ...
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-base">Timeline recente</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  {timeline.map((event) => (
                    ...
                    >
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {event.title.startsWith("Documento ") && (
                          <DocumentStatusBadge status={event.title.slice(10)} />
                        )}
                      </div>
                      <p className="text-xs text-muted-foreground">
                        {event.detail}
                  ...
                  <Button asChild size="sm" variant="link" className="px-0">
                    <Link
                      href={`/dashboard/clienti/${customer.id}?tab=timeline`}
                    >
                      <CalendarClock />
                      Timeline completa
                    </Link>
          ...
          <Separator />
          <SheetFooter className="shrink-0 flex-row gap-2 px-5 py-3">
            <Button asChild className="flex-1">
              <Link
                data-flow-tour="flow-client-open-full"
