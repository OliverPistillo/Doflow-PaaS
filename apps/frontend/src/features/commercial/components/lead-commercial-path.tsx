...
import { quoteStatuses, type QuoteStatus } from "@/features/commercial/commercial-documents"
...
export function LeadCommercialPath({ lead }: { lead: CommercialLead }) {
  ...
  const proposalReady = Boolean(quote && !["Bozza", "Rifiutato", "Scaduto", "Sostituito"].includes(quote.status))
...
export function LeadDocumentCenter({ lead }: { lead: CommercialLead }) {
  ...
  const financial = order ? { paid: order.netCollected ?? 0 } : undefined
  const quote = [...store.quotes].filter((item) => item.leadId === lead.id && !item.archivedAt).sort((a, b) => b.updatedAt.localeCompare(a.updatedAt))[0]
  const setProposalStatus = async (status: QuoteStatus) => {
    if (!quote || quote.status === status) return
    if (await store.updateQuote(quote.id, { status })) toast.success(`Proposta ${status.toLowerCase()}`)
  }
  const createProposalVersion = async () => {
    if (!quote) return
    const result = await store.createQuoteVersion(quote.id)
    if (result.ok) toast.success(result.existing ? "Versione già presente" : "Nuova versione creata")
  ...
  const cards = [{ label: "Brief e materiali", status: lead.materialsStatus ?? "Da richiedere", icon: ReceiptText }, { label: "Proposta / preventivo", status: quote?.status ?? "Da preparare", icon: ReceiptText }, { label: "Contratto", status: contract?.status ?? "Da generare", icon: FileSignature }, { label: "Acconto", status: deposit, icon: ReceiptText }]
  return <Card id="documents"><CardHeader><CardTitle>Centro documentale</CardTitle><CardDescription>Metadati operativi letti e aggiornati tramite i moduli documentali server-backed.</CardDescription></CardHeader><CardContent className="space-y-4"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{cards.map(({ label, status, icon: Icon }) => <div key={label} className="rounded-lg border p-3"><div className="flex items-center gap-2"><Icon className="size-4 text-muted-foreground" /><strong className="text-sm">{label}</strong></div><DocumentStatusBadge className="mt-2" status={status} /></div>)}</div><div className="grid gap-4 lg:grid-cols-2"><div className="rounded-lg border p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><h3 className="font-medium">Proposta</h3><p className="text-sm text-muted-foreground">{quote ? `${quote.code} · v${quote.version} · ${money.format(quote.total)}` : "Nessuna proposta server registrata"}</p></div>{quote?.validUntil && <Badge variant="outline">Scade {date.format(new Date(quote.validUntil))}</Badge>}</div><div className="mt-3 flex flex-wrap gap-2">{!quote ? <Button asChild size="sm" variant="outline"><Link href="/dashboard/preventivi">Apri Preventivi per crearla</Link></Button> : <><Select value={quote.status} onValueChange={(value) => void setProposalStatus(value as QuoteStatus)} disabled={!editable}><SelectTrigger className="w-44"><SelectValue /></SelectTrigger><SelectContent>{quoteStatuses.map((status) => <SelectItem key={status} value={status}>{status}</SelectItem>)}</SelectContent></Select><Button size="sm" variant="outline" onClick={() => void createProposalVersion()} disabled={!editable}>Nuova versione</Button></>}</div></div><div className="rounded-lg border p-4"><h3 className="font-medium">Contratto e ordine</h3><div className="mt-1 flex flex-wrap items-center gap-2"><p className="text-sm text-muted-foreground">{contract ? `${contract.code} · v${contract.version}` : "Contratto non ancora generato"}</p>{contract && <DocumentStatusBadge status={contract.status} />}</div><p className="mt-1 text-sm text-muted-foreground">{order ? `${order.code} · acconto ${deposit}` : "Ordine non collegato"}</p><div className="mt-3 flex flex-wrap gap-2"><Button asChild size="sm" variant="outline"><Link href="/dashboard/contratti">Apri contratti</Link></Button>{order && <Button asChild size="sm" variant="outline"><Link href="/dashboard/ordini">Apri ordine</Link></Button>}</div></div></div>{customer?.documents?.length ? <div><h3 className="mb-2 text-sm font-medium">Altri documenti</h3><div className="flex flex-wrap gap-2">{customer.documents.filter((item) => !item.archivedAt).map((item) => <span key={item.id} className="inline-flex items-center gap-1 rounded-md border px-2 py-1 text-xs"><span>{item.name}</span><DocumentStatusBadge status={item.status} /></span>)}</div></div> : null}</CardContent></Card>
}
