...
export function AutomationPerformancePage() {
  ...
  const canViewRuns = identity.hasCapability("canViewAutomationErrors");
  const canViewAdapters = identity.currentUser.roles.includes("administrator");
  ...
  return <main className="mx-auto w-full max-w-7xl space-y-6 p-4 md:p-6" data-automation-authority="server">
    ...
    <Tabs defaultValue="automations"><TabsList className="flex h-auto w-full flex-wrap justify-start"><TabsTrigger value="automations">Automazioni</TabsTrigger><TabsTrigger value="points">Punti</TabsTrigger><TabsTrigger value="rankings">Classifiche</TabsTrigger><TabsTrigger value="mission">Missione</TabsTrigger>{canViewAdapters ? <TabsTrigger value="adapters">Adapter</TabsTrigger> : null}</TabsList>
