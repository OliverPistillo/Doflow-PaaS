...
type LeadActionsProps = { lead: CommercialLead; onSave: (updates: GeneralLeadFields) => void; onCreateActivity: () => void }
...
function notify(action: string) { toast.info(`${action} sarà disponibile nella prossima fase.`) }
...
export function LeadDetailTopActions({ lead, onSave, onCreateActivity }: LeadActionsProps) {
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setIsSaving(true)
    try {
      // Mantiene visibile lo stato di salvataggio mentre viene aggiornato lo stato condiviso della scheda.
      await new Promise((resolve) => window.setTimeout(resolve, 250))
      onSave({ ...draft, opportunityName: draft.opportunityName.trim(), firstName: draft.firstName.trim(), lastName: draft.lastName.trim(), company: draft.company.trim(), email: draft.email.trim(), phone: draft.phone.trim(), location: draft.location.trim() })
      setOpen(false)
  ...
  return <div className="flex items-center gap-2"><Button variant="outline" onClick={openEditor}><Pencil />Modifica</Button><DropdownMenu><DropdownMenuTrigger asChild><Button variant="outline">Azioni<ChevronDown /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onSelect={onCreateActivity}><CalendarPlus />Crea attività</DropdownMenuItem><DropdownMenuItem onClick={() => notify("Aggiungi nota")}><Plus />Aggiungi nota</DropdownMenuItem><DropdownMenuItem onClick={() => window.location.href = `mailto:${lead.email}`}><Mail />Invia email</DropdownMenuItem><DropdownMenuItem onClick={() => window.open(`https://wa.me/${lead.phone.replace(/\D/g, "")}`, "_blank", "noopener,noreferrer")}><MessageCircle />Apri WhatsApp</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem onClick={() => notify("Segna come vinto")}><Trophy />Segna come vinto</DropdownMenuItem><DropdownMenuItem variant="destructive" onClick={() => notify("Segna come perso")}><XCircle />Segna come perso</DropdownMenuItem></DropdownMenuContent></DropdownMenu>
