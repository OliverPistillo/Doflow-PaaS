...
export function LeadQuickSheet({ leadId, open, onOpenChange }: { leadId?: string; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const save = async () => {
    ...
    if (stage !== lead.stage) await store.transitionLeadStatus({ leadId: lead.id, fromStatus: lead.stage, toStatus: stage })
    store.updateLead(lead.id, { assigneeId, owner: commercialTeam.find((item) => item.id === assigneeId)?.name ?? lead.owner, nextAction: nextAction.trim(), nextActionAt: new Date(nextActionAt).toISOString() }, { silentTimeline: stage !== lead.stage })
