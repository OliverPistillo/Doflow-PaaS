...
import {
  customerActivityPriorities,
  customerActivityRecurrences,
...
export function ActivityFormDialog({
  ...
  const [priority, setPriority] =
    useState<CustomerActivity["priority"]>("Media");
  ...
  const [recurrence, setRecurrence] =
    useState<CustomerActivity["recurrence"]>("Nessuna");
  ...
  const effectiveProjectId = lockProject
    ? (defaultProjectId ?? "none")
    : projectId || defaultProjectId || "none";
  const customer = customers.find((item) => item.id === effectiveClientId);
  const lead = defaultLeadId
    ? leads.find((item) => item.id === defaultLeadId)
    : undefined;
  const clientProjects = useMemo(
    () => projects.filter((item) => item.clientId === effectiveClientId),
    [effectiveClientId, projects],
  );
  const project = clientProjects.find((item) => item.id === effectiveProjectId);
  const canAssignTeam =
    identity.hasCapability("canAssignLeads") ||
    identity.hasCapability("canManageProjects");
  const teamOptions = canAssignTeam
    ? commercialTeam
    : commercialTeam.filter((item) => item.id === identity.currentUserId);
  ...
  const reset = () => {
    ...
    setType("Attività");
    setPriority("Media");
    setAssigneeId(identity.currentUserId);
    setCollaboratorId("none");
    setStartAt("");
    setDueDate(defaultDueDate);
    setDueTime(defaultDueTime);
    setRecurrence("Nessuna");
  ...
  const save = async () => {
    ...
    const input = {
      ...
      priority,
      assigneeId: assigneeId || identity.currentUserId,
      collaboratorIds: collaboratorId === "none" ? [] : [collaboratorId],
      ...
      recurrence,
      dependencyIds: [],
      technicalCategory: technicalCategory.trim() || undefined,
      origin: "manuale" as const,
    };
    const activityId = await (customer
      ? store.addCustomerActivity(customer.id, {
          ...input,
          leadId: customer.sourceLeadId,
        })
      : store.addLeadActivity(lead!.id, input));
    savingRef.current = false;
    setSaving(false);
    if (!activityId) {
      toast.error("Impossibile creare l’attività nel perimetro corrente.");
      return;
    }
      ...
      <DialogContent className="flex max-h-[92dvh] flex-col overflow-hidden sm:max-w-2xl">
        ...
        <div className="grid min-h-0 gap-4 overflow-y-auto pr-1 sm:grid-cols-2">
          {lead ? (
            ...
            <Label className="sm:col-span-2">
              ...
              <Select
                ...
                disabled={lockClient}
              >
                <SelectTrigger aria-label="Cliente attività">
          ...
          <Label className="sm:col-span-2">
            Titolo
            <Input
              aria-label="Titolo nuova attività"
              value={title}
          ...
          <Label>
            ...
            <Select
              value={type}
              onValueChange={(value) =>
                setType(value as CustomerActivity["type"])
              }
            >
              <SelectTrigger aria-label="Tipologia attività">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <Label>
            ...
            <Select
              value={priority}
              onValueChange={(value) =>
                setPriority(value as CustomerActivity["priority"])
              }
            >
              <SelectTrigger aria-label="Priorità attività">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {customerActivityPriorities.map((item) => (
                  <SelectItem key={item} value={item}>
                    {item}
                  </SelectItem>
                ))}
          ...
          <div className="grid min-w-0 gap-4 sm:col-span-2 lg:grid-cols-2">
            <div className="min-w-0 space-y-1.5">
              ...
              <Select value={assigneeId} onValueChange={setAssigneeId}>
                <SelectTrigger
                  className="!h-12 w-full min-w-0 px-2 [&>span]:min-w-0 [&>span]:flex-1"
                  aria-label="Responsabile attività"
                >
                  <PresenceUserOption userId={assigneeId} />
                </SelectTrigger>
                <SelectContent
                  position="popper"
                  align="start"
                  className="min-w-[min(320px,calc(100vw-2rem))]"
                >
                  {teamOptions.map((item) => (
                    <SelectItem key={item.id} value={item.id}>
                      <PresenceUserOption userId={item.id} />
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <div className="min-w-0 space-y-1.5">
              ...
              <Select value={collaboratorId} onValueChange={setCollaboratorId}>
                <SelectTrigger
                  className="!h-12 w-full min-w-0 px-2 [&>span]:min-w-0 [&>span]:flex-1"
                  aria-label="Collaboratore attività"
                >
                  {collaboratorId === "none" ? (
                    ...
                    <PresenceUserOption userId={collaboratorId} />
                  )}
                </SelectTrigger>
                <SelectContent
                  position="popper"
                ...
                >
                  <SelectItem value="none">
                    <span className="flex h-7 items-center">Nessuno</span>
                  </SelectItem>
                  {commercialTeam
                    .filter((item) => item.id !== assigneeId)
                    .map((item) => (
                      <SelectItem key={item.id} value={item.id}>
                        <PresenceUserOption userId={item.id} />
                      </SelectItem>
          ...
          <Label>
            ...
            <Select
              ...
              disabled={Boolean(lead) || lockProject || !effectiveClientId}
            >
              <SelectTrigger aria-label="Progetto attività">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <Label>
            ...
            <Select
              ...
              disabled={!project}
            >
              <SelectTrigger aria-label="Fase del progetto">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <Label>
            Scadenza
            <Input
              aria-label="Scadenza nuova attività"
              type="date"
              value={dueDate}
          ...
          <Label>
            ...
            <Select
              value={recurrence}
              onValueChange={(value) =>
                setRecurrence(value as CustomerActivity["recurrence"])
              }
            >
              <SelectTrigger aria-label="Ricorrenza attività">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {customerActivityRecurrences.map((item) => (
                  <SelectItem key={item} value={item}>
                    {item}
                  </SelectItem>
                ))}
          ...
          <Label className="sm:col-span-2">
            ...
            <Input
              ...
              onChange={(event) => setTechnicalCategory(event.target.value)}
              placeholder="Facoltativa, es. frontend, API, contenuti"
            />
          </Label>
        </div>
        <DialogFooter className="shrink-0 border-t pt-3">
          ...
          <Button
            disabled={(!customer && !lead) || !title.trim() || saving}
            onClick={save}
          >
