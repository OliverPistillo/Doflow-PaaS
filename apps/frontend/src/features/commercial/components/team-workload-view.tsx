...
export function TeamWorkloadView() {
  ...
  const baseRows = authorizedUsers.map((user) => {
    ...
    const recordedMinutes = store.timeSessions.filter((session) => !session.archivedAt && session.userId === user.id && projectIds.has(session.projectId) && getRomeDateKey(session.startedAt) >= range.start && getRomeDateKey(session.startedAt) <= range.end).reduce((sum, session) => sum + sessionMinutes(session.startedAt, session.endedAt, session.durationMinutes), 0)
...
function UserAvatar({ user }: { user: DoflowIdentityUser }) { return <Avatar size="lg"><AvatarImage src={user.avatarUrl} alt={`Avatar di ${user.name}`} /><AvatarFallback>{initials(user.name)}</AvatarFallback></Avatar> }
