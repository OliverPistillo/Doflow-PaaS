...
import { inPeriod, loadRules, loadRuns, numeric, successRate, useAutomationCenterAccess } from "./automation-center-model";
...
export function AutomationReportsWorkspace() {
  const { canViewReports, canViewRules, canViewRuns } = useAutomationCenterAccess();
  ...
  const load = useCallback(async () => {
    if (!canViewReports) { setLoading(false); return; } setLoading(true); setError(null);
    try {
      const automationAllowed = canViewRules && canViewRuns;
  ...
  }, [canViewReports, canViewRules, canViewRuns, days]);
  useEffect(() => {
    let cancelled = false;
    queueMicrotask(() => {
      if (!cancelled) {
  ...
  if (!canViewReports) return <AccessDenied resource="ai report automazioni" />;
  return <main className="space-y-5 px-4 py-6 sm:px-6 lg:px-8">
    ...
    {loading ? <AutomationLoading /> : <>
      {!(canViewRules && canViewRuns) ? <p className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">I target Reports sono disponibili, ma le metriche di esecuzione richiedono accesso al monitoraggio Automations.</p> : null}
