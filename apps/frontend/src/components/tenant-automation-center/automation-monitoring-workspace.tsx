...
import { formatDateTime, formatDuration, formatTime, inPeriod, isToday, loadRules, loadRuns, runStatus, triggerLabel } from "./automation-center-model";
...
export function AutomationMonitoringWorkspace() {
  ...
  const load = async (manual = false) => {
    if (!canView("automations")) { setLoading(false); return; } if (manual) setRefreshing(true); else setLoading(true); setError(null);
    try {
      const [summaryData, runData, ruleData, notificationData] = await Promise.all([
        automationsApi.summary(), loadRuns(), loadRules(), canView("notifications") ? listTenantNotifications({ limit: 20 }) : Promise.resolve({ items: [] as TenantNotification[] }),
      ]);
  ...
  return <main className="space-y-5 px-4 py-6 sm:px-6 lg:px-8">
    ...
    {loading ? <AutomationLoading /> : <>
      ...
      <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <div className="space-y-5"><AutomationProblemsPanel runs={periodRuns} rules={rules} />{canView("notifications") ? <AutomationNotificationsPanel notifications={notifications} importantOnly={importantOnly} onImportantChange={setImportantOnly} /> : null}</div>
      </div>
    </>}
  </main>;
}
