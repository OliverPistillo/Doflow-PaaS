...
export function AutomationPageHeader({ title, description, ctaLabel, ctaHref, canCreate = true, children }: {
...
export function AutomationKpi({ icon: Icon, label, value, hint, tone = "violet" }: {
  icon: LucideIcon; label: string; value: React.ReactNode; hint?: React.ReactNode; tone?: keyof typeof iconTones;
}) {
  return <article className="min-h-[132px] rounded-2xl border border-slate-200/80 bg-white p-5"><div className="flex items-start gap-4">
    <span className={cn("flex h-12 w-12 shrink-0 items-center justify-center rounded-full", iconTones[tone])}><Icon className="h-5 w-5" /></span>
...
export function AutomationPanel({ title, actionHref, actionLabel, className, children, header }: {
  title: string; actionHref?: string; actionLabel?: string; className?: string; children: React.ReactNode; header?: React.ReactNode;
}) {
  return <section className={cn("rounded-2xl border border-slate-200/80 bg-white p-5", className)}>
    <div className="mb-4 flex items-center justify-between gap-3"><h2 className="text-[17px] font-semibold text-slate-950">{title}</h2>{header}{actionHref && actionLabel ? <Link href={actionHref} className="inline-flex items-center gap-1 text-sm font-medium text-indigo-600 hover:text-indigo-700">{actionLabel}<ArrowUpRight className="h-3.5 w-3.5" /></Link> : null}</div>{children}
  </section>;
}

export function AutomationEmpty({ children, className }: { children: React.ReactNode; className?: string }) {
...
export function AutomationLoading() { return <div className="flex min-h-36 items-center justify-center text-sm text-slate-500"><Loader2 className="mr-2 h-4 w-4 animate-spin" />Caricamento…</div>; }
export function AutomationError({ message }: { message?: string | null }) { return message ? <div className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">{message}</div> : null; }
export function AutomationBadge({ label, tone = "slate" }: { label: React.ReactNode; tone?: keyof typeof iconTones }) {
  const classes = { violet: "border-violet-200 bg-violet-50 text-violet-700", blue: "border-blue-200 bg-blue-50 text-blue-700", green: "border-emerald-200 bg-emerald-50 text-emerald-700", orange: "border-amber-200 bg-amber-50 text-amber-700", red: "border-rose-200 bg-rose-50 text-rose-700", slate: "border-slate-200 bg-slate-50 text-slate-600" }[tone];
  return <span className={cn("inline-flex rounded-full border px-2.5 py-1 text-xs font-medium", classes)}>{label}</span>;
}
