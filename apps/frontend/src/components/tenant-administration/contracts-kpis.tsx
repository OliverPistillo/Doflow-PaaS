...
export function ContractsKpis({
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      <AdministrationKpi icon={FileCheck2} label="Attivi" value={data.signedContracts || rows.filter((row) => ["signed", "active"].includes(String(row.status || ""))).length} hint="Firmati o attivi" />
      <AdministrationKpi icon={FileSignature} label="In firma" value={data.waitingSignatureContracts || rows.filter((row) => ["internal_pending", "client_pending", "partially_signed"].includes(String(row.signature_status || ""))).length} hint="Con firma in corso" tone="orange" />
