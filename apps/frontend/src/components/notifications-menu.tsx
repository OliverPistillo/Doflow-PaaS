...
import { Bell, Building2, CalendarClock, CheckCheck, ClipboardCheck, Clock3, FolderKanban, UserRound } from "lucide-react"
...
export function NotificationsMenu({ open: controlledOpen, onOpenChange }: { open?: boolean; onOpenChange?: (open: boolean) => void } = {}) {
  ...
  const { notifications, summary, loading, markRead, markAllRead, markSeen } = useDoflowNotifications()
  const setOpen = (value: boolean) => { if (controlledOpen === undefined) setInternalOpen(value); onOpenChange?.(value); if (value) void markSeen() }
  const unread = summary.unreadNotifications
  const fresh = summary.newNotifications
  const recent = notifications.filter((item) => !item.archived).slice(0, 5)
  const headerCopy = presence.quietNotifications ? `${unread} non lette · avvisi non urgenti silenziati` : unread > 0 ? `${unread} notifiche non lette` : notifications.length > 0 ? "Nessuna notifica non letta." : "Nessuna nuova notifica"
  ...
  return <Popover open={open} onOpenChange={setOpen}>
    <Tooltip><TooltipTrigger asChild><PopoverTrigger asChild><Button variant="ghost" className="relative inline-flex h-9 shrink-0 items-center gap-1.5 px-2" aria-label="Apri notifiche"><Bell className="size-4 shrink-0" />{fresh > 0 && <><span className="hidden h-[18px] min-w-5 items-center justify-center rounded-full bg-red-700/90 px-1.5 text-[10px] font-semibold leading-none text-white sm:inline-flex">{fresh > 99 ? "99+" : fresh}</span><span className="absolute right-1.5 top-1.5 size-2 rounded-full bg-red-600 ring-2 ring-background sm:hidden" aria-label={`${fresh} nuove notifiche`} /></>}</Button></PopoverTrigger></TooltipTrigger><TooltipContent>Notifiche</TooltipContent></Tooltip>
