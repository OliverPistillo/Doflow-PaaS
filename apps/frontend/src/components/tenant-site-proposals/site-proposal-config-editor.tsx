...
function Field({ config, path, label, multiline, onChange }: { config: SiteConfig; path: string; label: string; multiline?: boolean; onChange: (next: SiteConfig) => void }) {
  ...
  return <div><Label htmlFor={path}>{label}</Label>{multiline ? <Textarea {...common} className="mt-1 min-h-24" /> : <Input {...common} className="mt-1" />} {limit ? <p className="mt-1 text-right text-xs text-muted-foreground">{current.length}/{limit}</p> : null}</div>;
}
...
export function SiteProposalConfigEditor({ config, onChange }: { config: SiteConfig; onChange: (next: SiteConfig) => void }) {
  ...
  return <div className="space-y-5">
    ...
    <CommercialSectionCard title="Immagini"><div className="grid gap-4 lg:grid-cols-3">{visualSlots.map((slot) => { const image = images[slot] as JsonObject; return <div key={slot} className="rounded-xl border p-4">{image.src ? <Image unoptimized width={640} height={144} src={String(image.src)} alt={String(image.alt || slot)} className="mb-3 h-36 w-full rounded-lg object-cover"/> : null}<Field config={config} path={`images.${slot}.src`} label={slot} onChange={(next) => changeImage(slot, next)}/><Field config={config} path={`images.${slot}.alt`} label="Testo alternativo" onChange={onChange}/><Field config={config} path={`images.${slot}.objectPosition`} label="Posizione immagine" onChange={onChange}/><p className="mt-2 text-xs text-muted-foreground">Metodo: {String(image.sourceMethod || "package")}</p></div>; })}</div></CommercialSectionCard>
    <CommercialSectionCard title="Contenuti del tema"><p className="mb-4 text-sm text-muted-foreground">Campi generati dal contratto del profilo {profile || "del tema"}. Le sezioni protette non sono modificabili.</p><div className="grid gap-4 lg:grid-cols-2">{contentPaths.map((path) => <Field key={path} config={config} path={path} label={labelFor(path)} multiline={/description|text|quote|notice|copyright/i.test(path)} onChange={onChange}/>)}</div></CommercialSectionCard>
