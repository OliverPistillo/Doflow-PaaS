...
function PreviewFrame({ html, viewport }: { html: string; viewport: Viewport }) {
  ...
  return <div ref={containerRef} className="overflow-hidden rounded-xl border border-border bg-muted p-3"><p className="mb-2 text-xs font-medium text-muted-foreground">{frame.label}</p><div className="origin-top-left" style={{ width: frame.width, height: frame.height * scale, transform: `scale(${scale})`, transformOrigin: "top left" }}><iframe title={`Anteprima sito ${frame.label}`} srcDoc={html} sandbox="allow-scripts" referrerPolicy="no-referrer" loading="lazy" className="block border-0 bg-card" style={{ width: frame.width, height: frame.height }} /></div></div>;
}
...
export function SiteProposalPreview({ id, generationId, comparison = false, onGenerate }: { id: string; generationId?: string; comparison?: boolean; onGenerate?: () => void }) {
  ...
  if (progress) return <CommercialSectionCard title="Anteprima"><SiteProposalProgress value={progress} /><p className="mt-3 text-sm text-muted-foreground">L’anteprima si aprirà automaticamente quando HTML e ZIP saranno pronti.</p></CommercialSectionCard>;
  if (loading && !html) return <CommercialSectionCard title="Anteprima"><div className="flex min-h-48 items-center justify-center text-sm text-muted-foreground"><Loader2 className="mr-2 h-4 w-4 animate-spin" />Caricamento anteprima…</div></CommercialSectionCard>;
  ...
  return <CommercialSectionCard title={compare ? "Confronto Mobile / Desktop" : "Anteprima sito"}><div className="mb-4 flex flex-wrap items-center gap-2"><Button size="sm" variant="outline" onClick={() => void load()} disabled={loading}><RefreshCw className="mr-2 h-4 w-4" />Aggiorna</Button>{!compare ? <><Button size="sm" variant={viewport === "mobile" ? "default" : "outline"} onClick={() => setViewport("mobile")}><Smartphone className="mr-1 h-4 w-4" />Mobile</Button><Button size="sm" variant={viewport === "tablet" ? "default" : "outline"} onClick={() => setViewport("tablet")}><Tablet className="mr-1 h-4 w-4" />Tablet</Button><Button size="sm" variant={viewport === "desktop" ? "default" : "outline"} onClick={() => setViewport("desktop")}><Monitor className="mr-1 h-4 w-4" />Desktop</Button></> : null}<Button size="sm" variant={compare ? "default" : "outline"} onClick={() => setCompare((value) => !value)}><Columns2 className="mr-1 h-4 w-4" />Confronta Mobile/Desktop</Button><span className="ml-auto text-xs text-muted-foreground">Le modifiche saranno visibili dopo Salva e genera.</span></div>{compare ? <div className="grid gap-4 2xl:grid-cols-2"><PreviewFrame html={html} viewport="mobile" /><PreviewFrame html={html} viewport="desktop" /></div> : <PreviewFrame html={html} viewport={viewport} />}</CommercialSectionCard>;
}
