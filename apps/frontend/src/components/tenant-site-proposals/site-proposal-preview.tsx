...
export function SiteProposalPreview({ id, generationId, comparison = false, onGenerate }: { id: string; generationId?: string; comparison?: boolean; onGenerate?: () => void }) {
  const [html, setHtml] = useState<string | null>(null); const [loading, setLoading] = useState(false); const [error, setError] = useState<string | null>(null); const [viewport, setViewport] = useState<Viewport>("mobile");
  ...
  return <CommercialSectionCard title={comparison ? "Confronto Mobile / Desktop" : "Anteprima sito"}><div className="mb-4 flex flex-wrap items-center gap-2"><Button size="sm" variant="outline" onClick={() => void load()} disabled={loading}><RefreshCw className="mr-2 h-4 w-4" />Aggiorna</Button>{!comparison ? <><Button size="sm" variant={viewport === "mobile" ? "default" : "outline"} onClick={() => setViewport("mobile")}><Smartphone className="mr-1 h-4 w-4" />Mobile</Button><Button size="sm" variant={viewport === "tablet" ? "default" : "outline"} onClick={() => setViewport("tablet")}><Tablet className="mr-1 h-4 w-4" />Tablet</Button><Button size="sm" variant={viewport === "desktop" ? "default" : "outline"} onClick={() => setViewport("desktop")}><Monitor className="mr-1 h-4 w-4" />Desktop</Button></> : null}<span className="ml-auto text-xs text-slate-500">Le modifiche saranno visibili dopo Salva e genera.</span></div>{comparison ? <div className="grid gap-4 2xl:grid-cols-2"><PreviewFrame html={html} viewport="mobile" /><PreviewFrame html={html} viewport="desktop" /></div> : <PreviewFrame html={html} viewport={viewport} />}</CommercialSectionCard>;
}
