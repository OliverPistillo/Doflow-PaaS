...
export function SiteProposalThemes() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { setThemes(await listProposalThemes()); }
  ...
  const runAction = async () => {
    ...
    try {
      ...
      if (kind === "delete") await deleteProposalTheme(theme.slug, theme.version);
      toast.success(kind === "delete" ? "Tema eliminato." : "Tema aggiornato."); setPending(null); await load();
    } catch (error) { toast.error(getErrorMessage(error)); }
  ...
  return <main className="space-y-5 px-4 py-6 sm:px-6 lg:px-8">
    ...
    <CommercialSectionCard title="Temi disponibili">
      <div className="mb-4 flex justify-end"><Button variant="outline" onClick={() => void load()} disabled={loading}><RefreshCw className="mr-2 h-4 w-4" />Aggiorna</Button></div>
      {loading ? <p className="py-8 text-center text-sm text-slate-500">Caricamento temi…</p> : themes.length === 0 ? <CommercialEmptyState>Nessun tema disponibile.</CommercialEmptyState> : <div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-left text-sm"><thead className="border-b text-xs uppercase text-slate-500"><tr><th className="p-3">Tema</th><th className="p-3">Versione</th><th className="p-3">Sorgente</th><th className="p-3">Categorie</th><th className="p-3">Stato</th><th className="p-3">Utilizzi</th><th className="p-3">Caricamento</th><th className="p-3" /></tr></thead><tbody>{themes.map((theme) => {
        const isDefault = theme.default_version === theme.version; const canDelete = canAdmin && !theme.is_builtin && !isDefault && theme.status !== "active" && Number(theme.usages || 0) === 0;
        ...
        return <tr key={`${theme.slug}@${theme.version}`} className="border-b last:border-0"><td className="p-3"><p className="font-medium text-slate-950">{theme.name}</p><p className="text-xs text-slate-500">{theme.slug} · {theme.content_profile}</p><div className="mt-1 flex flex-wrap gap-1"><Badge variant="outline">{theme.source_format === "modular" ? "Modulare" : "Legacy"}</Badge><Badge className={pendingAdapter ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-700"}>{pendingAdapter ? "Adattatore in preparazione" : "Adattatore attivo"}</Badge></div>{pendingAdapter ? <p className="mt-1 max-w-xs text-xs text-amber-700">Sarà attivato dopo l&apos;integrazione del profilo di generazione.</p> : null}</td><td className="p-3">{theme.version}{isDefault ? <Badge className="ml-2 bg-indigo-100 text-indigo-700">Predefinita</Badge> : null}<p className="mt-1 text-xs text-slate-500">format {theme.format_version || "—"}</p></td><td className="p-3">{theme.source_kind === "builtin" ? "Built-in" : "Upload"}<p className="mt-1 text-xs text-slate-500">Package {formatBytes(theme.zip_size)} · Compiled {formatBytes(theme.compiled_size)}</p><p className="max-w-[170px] truncate text-xs text-slate-400" title={theme.template_sha256}>src {theme.template_sha256?.slice(0, 12)}…</p><p className="max-w-[170px] truncate text-xs text-slate-400" title={theme.compiled_sha256 || undefined}>compiled {theme.compiled_sha256?.slice(0, 12) || "—"}</p></td><td className="p-3"><div className="flex max-w-[300px] flex-wrap gap-1">{(theme.categories || []).map((category) => <Badge key={category} variant="outline">{category}</Badge>)}{tags.map((tag) => <Badge key={tag} className="bg-slate-100 text-slate-600">{tag}</Badge>)}</div><p className="mt-1 text-xs text-slate-500">Conteggi: {summary(counts)} · Feature: {summary(features)} · Asset: {Object.keys(assets).length}</p></td><td className="p-3"><Badge className={theme.status === "active" ? "bg-emerald-100 text-emerald-700" : theme.status === "draft" ? "bg-amber-100 text-amber-700" : "bg-slate-100 text-slate-600"}>{theme.status}</Badge></td><td className="p-3">{theme.usages || 0}</td><td className="p-3 text-slate-500">{formatDate(theme.version_created_at)}</td><td className="p-3"><div className="flex justify-end gap-1"><Button size="icon" variant="ghost" aria-label="Anteprima" onClick={() => void preview(theme)}><Eye className="h-4 w-4" /></Button>{!theme.is_builtin || theme.source_format === "modular" ? <Button size="icon" variant="ghost" aria-label="Download ZIP" onClick={() => void download(theme)}><Download className="h-4 w-4" /></Button> : null}{canAdmin && theme.status !== "active" ? <Button size="sm" variant="outline" onClick={() => setPending({ kind: "activate", theme })}>Attiva</Button> : null}{canAdmin && theme.status === "active" && !theme.is_builtin && !isDefault ? <Button size="sm" variant="outline" onClick={() => setPending({ kind: "disable", theme })}>Disattiva</Button> : null}{canAdmin && theme.status === "active" && !isDefault ? <Button size="sm" variant="outline" disabled={pendingAdapter} title={pendingAdapter ? "Adattatore di generazione non ancora attivo" : undefined} onClick={() => setPending({ kind: "default", theme })}>Imposta predefinito</Button> : null}{canDelete ? <Button size="icon" variant="ghost" className="text-rose-600" aria-label="Elimina tema" onClick={() => setPending({ kind: "delete", theme })}><Trash2 className="h-4 w-4" /></Button> : null}</div></td></tr>;
      })}</tbody></table></div>}
    ...
    <AlertDialog open={Boolean(pending)} onOpenChange={(open) => !open && setPending(null)}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Conferma azione tema</AlertDialogTitle><AlertDialogDescription>{pending ? `${pending.kind === "delete" ? "Elimina" : pending.kind === "default" ? "Imposta come predefinita" : pending.kind === "activate" ? "Attiva" : "Disattiva"} la versione ${pending.theme.slug}@${pending.theme.version}?` : ""}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Annulla</AlertDialogCancel><AlertDialogAction disabled={busy} onClick={() => void runAction()}>{busy ? "Operazione…" : "Conferma"}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
  </main>;
}
