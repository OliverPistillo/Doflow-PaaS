  ...
  return <main className="space-y-5 px-4 py-6 sm:px-6 lg:px-8">
    ...
    <CommercialSectionCard title="Temi disponibili">
      ...
      {loading ? <p className="py-8 text-center text-sm text-slate-500">Caricamento temi…</p> : visibleThemes.length === 0 ? <CommercialEmptyState>Nessun tema disponibile.</CommercialEmptyState> : <div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-left text-sm"><thead className="border-b text-xs uppercase text-slate-500"><tr><th className="p-3">Tema</th><th className="p-3">Versione</th><th className="p-3">Sorgente</th><th className="p-3">Categorie</th><th className="p-3">Stato</th><th className="p-3">Utilizzi</th><th className="p-3">Caricamento</th><th className="p-3" /></tr></thead><tbody>{visibleThemes.map((theme) => {
        const isDefault = theme.isDefault ?? theme.default_version === theme.version; const uploaded = (theme.sourceType || theme.source_kind) === "uploaded"; const canDelete = canAdmin && uploaded && (theme.canDelete ?? true);
        ...
        return <tr key={`${theme.slug}@${theme.version}`} className="border-b last:border-0">
          ...
          <td className="p-3"><Badge variant="outline">{uploaded ? "Caricato" : "Built-in"}</Badge><p className="mt-2 text-xs text-slate-500">Immagini: {theme.default_image_mode === "theme" ? "tema" : "ibrida"}</p><p className="mt-1 text-xs text-slate-500">Package {formatBytes(theme.zip_size)} · Compiled {formatBytes(theme.compiled_size)}</p></td>
          ...
          <td className="p-3"><Badge className={theme.deleted_at || theme.status === "retired" ? "bg-rose-100 text-rose-700" : theme.status === "active" ? "bg-emerald-100 text-emerald-700" : theme.status === "draft" ? "bg-amber-100 text-amber-700" : "bg-slate-100 text-slate-600"}>{theme.deleted_at || theme.status === "retired" ? "Ritirato" : theme.status === "active" ? "Attivo" : theme.status === "disabled" ? "Disattivato" : "Bozza"}</Badge></td>
          <td className="p-3">{theme.usageCount ?? theme.usages ?? 0}<p className="text-xs text-slate-500">Storico: {theme.historicalUsageCount ?? 0}</p></td><td className="p-3 text-slate-500">{formatDate(theme.version_created_at)}</td>
