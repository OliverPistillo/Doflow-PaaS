...
export function SiteProposalThemes() {
  ...
  const uploadFile = async (file?: File) => {
    ...
    if (file.size > 5 * 1024 * 1024) { setUploadError("Il file supera il limite di 5 MiB."); return; }
    setUploading(true);
  ...
  return <main className="space-y-5 px-4 py-6 sm:px-6 lg:px-8">
    ...
    {canAdmin ? <CommercialSectionCard title="Carica tema ZIP">
      <div onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={drop} className={`rounded-2xl border-2 border-dashed p-8 text-center ${dragging ? "border-indigo-500 bg-indigo-50" : "border-slate-300 bg-slate-50"}`}>
        <Upload className="mx-auto h-7 w-7 text-slate-500" /><p className="mt-3 text-sm font-medium text-slate-900">Trascina qui il pacchetto oppure selezionalo</p><p className="mt-1 text-xs text-slate-500">ZIP standalone, massimo 5 MiB. La nuova versione resta in bozza.</p>
    ...
    <CommercialSectionCard title="Temi disponibili">
      ...
      {loading ? <p className="py-8 text-center text-sm text-slate-500">Caricamento temi…</p> : themes.length === 0 ? <CommercialEmptyState>Nessun tema disponibile.</CommercialEmptyState> : <div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-left text-sm"><thead className="border-b text-xs uppercase text-slate-500"><tr><th className="p-3">Tema</th><th className="p-3">Versione</th><th className="p-3">Sorgente</th><th className="p-3">Categorie</th><th className="p-3">Stato</th><th className="p-3">Utilizzi</th><th className="p-3">Caricamento</th><th className="p-3" /></tr></thead><tbody>{themes.map((theme) => {
        ...
        return <tr key={`${theme.slug}@${theme.version}`} className="border-b last:border-0"><td className="p-3"><p className="font-medium text-slate-950">{theme.name}</p><p className="text-xs text-slate-500">{theme.slug} · {theme.content_profile}</p></td><td className="p-3">{theme.version}{isDefault ? <Badge className="ml-2 bg-indigo-100 text-indigo-700">Predefinita</Badge> : null}</td><td className="p-3">{theme.source_kind === "builtin" ? "Built-in" : "Upload"}</td><td className="p-3"><div className="flex max-w-[260px] flex-wrap gap-1">{(theme.categories || []).map((category) => <Badge key={category} variant="outline">{category}</Badge>)}</div></td><td className="p-3"><Badge className={theme.status === "active" ? "bg-emerald-100 text-emerald-700" : theme.status === "draft" ? "bg-amber-100 text-amber-700" : "bg-slate-100 text-slate-600"}>{theme.status}</Badge></td><td className="p-3">{theme.usages || 0}</td><td className="p-3 text-slate-500">{formatDate(theme.version_created_at)}</td><td className="p-3"><div className="flex justify-end gap-1"><Button size="icon" variant="ghost" aria-label="Anteprima" onClick={() => void preview(theme)}><Eye className="h-4 w-4" /></Button>{!theme.is_builtin ? <Button size="icon" variant="ghost" aria-label="Download ZIP" onClick={() => void download(theme)}><Download className="h-4 w-4" /></Button> : null}{canAdmin && theme.status !== "active" ? <Button size="sm" variant="outline" onClick={() => setPending({ kind: "activate", theme })}>Attiva</Button> : null}{canAdmin && theme.status === "active" && !theme.is_builtin && !isDefault ? <Button size="sm" variant="outline" onClick={() => setPending({ kind: "disable", theme })}>Disattiva</Button> : null}{canAdmin && theme.status === "active" && !isDefault ? <Button size="sm" variant="outline" onClick={() => setPending({ kind: "default", theme })}>Imposta predefinito</Button> : null}{canDelete ? <Button size="icon" variant="ghost" className="text-rose-600" aria-label="Elimina tema" onClick={() => setPending({ kind: "delete", theme })}><Trash2 className="h-4 w-4" /></Button> : null}</div></td></tr>;
      })}</tbody></table></div>}
