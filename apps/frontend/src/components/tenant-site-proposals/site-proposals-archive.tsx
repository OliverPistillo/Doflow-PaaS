...
export function SiteProposalsArchive() {
  ...
  return <main className="space-y-5 px-4 py-6 sm:px-6 lg:px-8">
    ...
    <CommercialSectionCard title="Proposte archiviate">
      <div className="mb-4 grid gap-3 md:grid-cols-[1fr_220px_auto]"><div className="relative"><Search className="absolute left-3 top-3.5 h-4 w-4 text-muted-foreground" /><Input value={search} onChange={(event) => setSearch(event.target.value)} className="h-11 pl-9" placeholder="Cerca proposta archiviata" aria-label="Cerca proposta archiviata" /></div><Select value={template} onValueChange={(value) => { setTemplate(value); setOffset(0); setSelectedIds(new Set()); }}><SelectTrigger className="h-11"><SelectValue placeholder="Template" /></SelectTrigger><SelectContent><SelectItem value="all">Tutti i template</SelectItem>{templates.map((item) => <SelectItem key={item.slug} value={item.slug}>{item.name}</SelectItem>)}</SelectContent></Select><Button variant="outline" className="h-11" onClick={() => void load()} disabled={loading}><RefreshCw className="mr-2 h-4 w-4" />Aggiorna</Button></div>
      {selectedIds.size > 0 ? <div className="mb-4 flex flex-wrap items-center gap-2 rounded-xl border border-primary/30 bg-primary/10 p-3"><strong className="mr-auto text-sm text-foreground">{selectedIds.size} proposte selezionate</strong>{canRestore ? <Button size="sm" variant="outline" onClick={() => setRestoreTarget("bulk")}><ArchiveRestore className="mr-2 h-4 w-4" />Ripristina</Button> : null}{canPermanentlyDelete ? <Button size="sm" variant="destructive" onClick={() => { setDeleteConfirmation(""); setDeleteTarget("bulk"); }}><Trash2 className="mr-2 h-4 w-4" />Elimina definitivamente</Button> : null}<Button size="sm" variant="ghost" onClick={() => setSelectedIds(new Set())}>Deseleziona</Button></div> : null}
      ...
      {loading ? <div className="space-y-2">{Array.from({ length: 5 }).map((_, index) => <Skeleton key={index} className="h-16 w-full" />)}</div> : items.length === 0 ? <CommercialEmptyState>Nessuna proposta archiviata.</CommercialEmptyState> : <>
        <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[980px] text-left text-sm"><thead className="border-b text-xs uppercase text-muted-foreground"><tr><th className="p-3"><Checkbox aria-label="Seleziona tutte le proposte archiviate della pagina" checked={allPageSelected ? true : somePageSelected ? "indeterminate" : false} onCheckedChange={(checked) => togglePage(checked === true)} /></th><th className="p-3">Proposta</th><th className="p-3">Template</th><th className="p-3">Stato precedente</th><th className="p-3">Versione</th><th className="p-3">Data archiviazione</th><th className="p-3">Ultima generazione</th><th className="p-3">Azioni</th></tr></thead><tbody>{items.map((item) => { const selected = selectedIds.has(item.id); return <tr key={item.id} className={`border-b last:border-0 ${selected ? "bg-primary/10" : ""}`}><td className="p-3"><Checkbox aria-label={`Seleziona ${item.display_name}`} checked={selected} onCheckedChange={(checked) => toggle(item.id, checked === true)} /></td><td className="p-3 font-medium text-foreground">{item.display_name}</td><td className="p-3">Tema Colsova <span className="text-xs text-muted-foreground">{item.template_version}</span></td><td className="p-3"><Badge className="bg-muted text-muted-foreground">{proposalStatusLabel[item.archived_from_status || ""] || (item.last_generated_at ? "Generata" : "Bozza")}</Badge></td><td className="p-3">v{item.current_version}</td><td className="p-3 text-muted-foreground">{formatDate(item.deleted_at)}</td><td className="p-3 text-muted-foreground">{formatDate(item.last_generated_at)}</td><td className="p-3"><div className="flex justify-end gap-1">{canRestore ? <Button size="icon" variant="ghost" aria-label={`Ripristina ${item.display_name}`} onClick={() => setRestoreTarget(item)}><ArchiveRestore className="h-4 w-4" /></Button> : null}{canPermanentlyDelete ? <Button size="icon" variant="ghost" className="text-rose-600" aria-label={`Elimina definitivamente ${item.display_name}`} onClick={() => setDeleteTarget(item)}><Trash2 className="h-4 w-4" /></Button> : null}</div></td></tr>; })}</tbody></table></div>
        <div className="grid gap-3 md:hidden">{items.map((item) => { const selected = selectedIds.has(item.id); return <article key={item.id} className={`rounded-xl border p-4 ${selected ? "border-primary/40 bg-primary/10" : "border-border"}`}><div className="flex items-start gap-3"><Checkbox aria-label={`Seleziona ${item.display_name}`} checked={selected} onCheckedChange={(checked) => toggle(item.id, checked === true)} /><div className="min-w-0 flex-1"><h2 className="font-semibold text-foreground">{item.display_name}</h2><p className="mt-1 text-xs text-muted-foreground">Tema Colsova · v{item.current_version}</p><p className="mt-2 text-xs text-muted-foreground">Stato precedente: {proposalStatusLabel[item.archived_from_status || ""] || (item.last_generated_at ? "Generata" : "Bozza")}</p><p className="mt-1 text-xs text-muted-foreground">Archiviata {formatDate(item.deleted_at)}</p><div className="mt-3 flex flex-wrap gap-2">{canRestore ? <Button size="sm" variant="outline" onClick={() => setRestoreTarget(item)}>Ripristina</Button> : null}{canPermanentlyDelete ? <Button size="sm" variant="destructive" onClick={() => setDeleteTarget(item)}>Elimina</Button> : null}</div></div></div></article>; })}</div>
