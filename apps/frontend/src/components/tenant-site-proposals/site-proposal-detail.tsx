...
export function SiteProposalDetail({ id }: { id: string }) {
  ...
  useEffect(() => { if (!preparationInProgress) return; let timer: number | undefined; let cancelled = false; const poll = async () => { await load(true); if (!cancelled) timer = window.setTimeout(poll, document.visibilityState === "hidden" ? 6000 : 1800); }; timer = window.setTimeout(poll, 1800); return () => { cancelled = true; if (timer) window.clearTimeout(timer); }; }, [load, preparationInProgress]);
  ...
  const changeImageMode = async (imageMode: NonNullable<typeof detail>["proposal"]["image_mode"]) => { if (!imageMode) return; try { await updateSiteProposal(id, { imageMode }); toast.success("Politica immagini salvata. Conferma la rigenerazione completa per applicarla."); await load(); } catch (value) { toast.error(getErrorMessage(value)); } };
  const resetThemeImages = async (slot?: "hero" | "consultation" | "feature") => { try { await updateSiteProposal(id, slot ? { resetThemeImageSlot: slot } : { applyThemeImages: true, imageMode: "theme" }); toast.success(slot ? "Immagine del tema ripristinata." : "Immagini del tema applicate."); setDirty(false); await load(); } catch (value) { toast.error(getErrorMessage(value)); } };
  ...
  return <main className="space-y-5 px-4 py-6 sm:px-6 lg:px-8">
    ...
    {activeTab === "dati" ? <><SiteProposalConfigEditor config={draft.siteConfig} onChange={(siteConfig) => updateDraft({ ...draft, siteConfig })} /><SiteProposalImages config={draft.siteConfig} imageMode={proposal.image_mode || "hybrid"} disabled={busy} onConfigChange={(siteConfig) => updateDraft({ ...draft, siteConfig })} onModeChange={(mode) => void changeImageMode(mode)} onReset={(slot) => void resetThemeImages(slot)} /></> : null}
    {activeTab === "sito" ? <SiteProposalPreview id={id} generationId={previewId} onGenerate={() => setPrepareOpen(true)} /> : null}
