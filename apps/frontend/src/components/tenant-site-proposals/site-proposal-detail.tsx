...
export function SiteProposalDetail({ id }: { id: string }) {
  ...
  return <main className="space-y-5 px-4 py-6 sm:px-6 lg:px-8">
    ...
    {!emailValid && !preparationInProgress ? <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900"><span>Preparazione incompleta: oggetto, corpo email e [LINK_DEMO] devono essere presenti.</span>{canManage("crm") ? <Button size="sm" variant="outline" onClick={() => setPrepareOpen(true)}>Rigenera proposta completa</Button> : null}</div> : null}
