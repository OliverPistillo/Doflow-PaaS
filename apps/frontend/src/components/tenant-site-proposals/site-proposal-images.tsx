...
export function SiteProposalImages({ config, imageMode, disabled, onConfigChange, onModeChange, onReset }: { config: SiteConfig; imageMode: ThemeImageMode; disabled?: boolean; onConfigChange: (value: SiteConfig) => void; onModeChange: (mode: ThemeImageMode) => void; onReset: (slot?: "hero" | "consultation" | "feature") => void }) {
  ...
  return <CommercialSectionCard title="Immagini della proposta">
    ...
    <p className="mb-4 text-xs text-slate-500">Il cambio modalità invalida la readiness. Le immagini verranno rivalutate soltanto dopo la conferma di “Rigenera proposta completa”.</p>
    <div className="grid gap-3 lg:grid-cols-3">{(["hero","consultation","feature"] as const).map((slot) => { const image = images[slot] || {}; const method = String(image.sourceMethod || ""); return <div key={slot} className="rounded-xl border border-slate-200 p-3"><div className="mb-2 flex items-center justify-between"><strong className="capitalize text-slate-900">{slot}</strong><Badge variant="outline">{sourceLabels[method] || method || "Tema"}</Badge></div>{typeof image.src === "string" && image.src ? <div className="mb-3 h-24 overflow-hidden rounded bg-slate-100"><Image unoptimized width={640} height={96} src={image.src} alt={String(image.alt || slot)} className="h-full w-full object-cover" referrerPolicy="no-referrer" /></div> : null}<div className="flex gap-2"><Input value={values[slot] || ""} onChange={(event) => setValues((current) => ({ ...current, [slot]: event.target.value }))} placeholder="https://…" disabled={disabled} /><Button type="button" size="sm" disabled={disabled} onClick={() => replace(slot)}>Sostituisci</Button></div><Button type="button" className="mt-2" size="sm" variant="ghost" disabled={disabled} onClick={() => onReset(slot)}>Ripristina immagine del tema</Button></div>; })}</div>
