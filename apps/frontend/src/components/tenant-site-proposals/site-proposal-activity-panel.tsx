...
}, [load]); return <CommercialSectionCard title="Attività">{error ? <p className="text-sm text-rose-600">{error}</p> : null}{!loading && items.length === 0 ? <CommercialEmptyState>Nessuna attività registrata.</CommercialEmptyState> : <ol className="space-y-4 border-l border-slate-200 pl-5">{items.map((item) => <li key={item.id} className="relative"><span className="absolute -left-[25px] top-1 h-3 w-3 rounded-full border-2 border-white bg-indigo-500" /><p className="text-sm font-medium text-slate-900">{labels[item.action] || "Attività proposta"}</p><p className="mt-1 text-xs text-slate-500">{formatDate(item.created_at)}{item.actor_email ? ` · ${item.actor_email}` : ""}</p>{safeMetadata(item.metadata) ? <p className="mt-1 text-xs text-slate-600">{safeMetadata(item.metadata)}</p> : null}</li>)}</ol>}{items.length < total ? <Button className="mt-5" variant="outline" disabled={loading} onClick={() => void load(items.length)}>Carica altre attività</Button> : null}</CommercialSectionCard>; }
