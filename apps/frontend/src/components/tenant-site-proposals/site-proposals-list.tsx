...
import { archiveSiteProposal, archiveSiteProposals, deleteSiteProposal, deleteSiteProposals, generateSiteProposal, listSiteProposals, listTemplates, type ProposalStatus, type SiteProposal, type SiteProposalTemplate } from "@/lib/tenant-site-proposals-api";
...
export function SiteProposalsList() {
  ...
  const generate = async (proposal: SiteProposal) => {
    setBusyId(proposal.id);
    try { await generateSiteProposal(proposal.id); toast.success("Proposta generata."); await load(); }
  ...
  return <main className="space-y-5 px-4 py-6 sm:px-6 lg:px-8">
    ...
    <CommercialSectionCard title="Elenco proposte">
      ...
      {loading ? <div className="space-y-2">{Array.from({ length: 5 }).map((_, index) => <Skeleton key={index} className="h-16 w-full" />)}</div> : items.length === 0 ? <CommercialEmptyState>Nessuna proposta corrisponde ai filtri selezionati.</CommercialEmptyState> : <>
        <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[920px] text-left text-sm"><thead className="border-b text-xs uppercase text-slate-500"><tr><th className="p-3"><Checkbox aria-label="Seleziona tutte le proposte della pagina" checked={allPageSelected ? true : somePageSelected ? "indeterminate" : false} onCheckedChange={(checked) => togglePage(checked === true)} /></th><th className="p-3">Proposta</th><th className="p-3">Template</th><th className="p-3">Stato</th><th className="p-3">Cliente CRM</th><th className="p-3">Versione</th><th className="p-3">Aggiornamento</th><th className="p-3" /></tr></thead><tbody>{items.map((item) => { const selected = selectedIds.has(item.id); return <tr key={item.id} className={`border-b last:border-0 ${selected ? "bg-indigo-50" : ""}`}><td className="p-3"><Checkbox aria-label={`Seleziona ${item.display_name}`} checked={selected} onCheckedChange={(checked) => toggle(item.id, checked === true)} /></td><td className="p-3"><Link className="font-medium text-slate-950 hover:text-indigo-600" href={`/commercial/site-proposals/${item.id}`}>{item.display_name}</Link><p className="mt-1 text-xs text-slate-500">{item.last_generated_at ? "Generata" : "Nessuna generazione"}</p></td><td className="p-3">Tema Colsova <span className="text-xs text-slate-500">{item.template_version}</span></td><td className="p-3"><Badge className={statusTone(item.status)}>{proposalStatusLabel[item.status] || item.status}</Badge></td><td className="p-3">{item.company_id ? "Collegata" : "—"}</td><td className="p-3">v{item.current_version}</td><td className="p-3 text-slate-500">{formatDate(item.updated_at)}</td><td className="p-3"><div className="flex justify-end gap-1"><Button asChild size="icon" variant="ghost" aria-label={`Apri ${item.display_name}`}><Link href={`/commercial/site-proposals/${item.id}`}><Eye className="h-4 w-4" /></Link></Button>{canManage("crm") ? <Button size="icon" variant="ghost" aria-label={`Genera ${item.display_name}`} disabled={busyId === item.id} onClick={() => void generate(item)}><Sparkles className="h-4 w-4" /></Button> : null}{canArchive ? <Button size="icon" variant="ghost" aria-label={`Archivia ${item.display_name}`} onClick={() => setArchiveTarget(item)}><Archive className="h-4 w-4" /></Button> : null}{canPermanentlyDelete ? <Button size="icon" variant="ghost" className="text-rose-600" aria-label={`Elimina definitivamente ${item.display_name}`} onClick={() => setDeleteTarget(item)}><Trash2 className="h-4 w-4" /></Button> : null}</div></td></tr>; })}</tbody></table></div>
        <div className="grid gap-3 md:hidden">{items.map((item) => { const selected = selectedIds.has(item.id); return <article key={item.id} className={`rounded-xl border p-4 ${selected ? "border-indigo-300 bg-indigo-50" : "border-slate-200"}`}><div className="flex items-start gap-3"><Checkbox aria-label={`Seleziona ${item.display_name}`} checked={selected} onCheckedChange={(checked) => toggle(item.id, checked === true)} /><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><div><Link className="font-semibold text-slate-950" href={`/commercial/site-proposals/${item.id}`}>{item.display_name}</Link><p className="mt-1 text-xs text-slate-500">Tema Colsova · v{item.current_version}</p></div><Badge className={statusTone(item.status)}>{proposalStatusLabel[item.status] || item.status}</Badge></div><p className="mt-3 text-xs text-slate-500">Aggiornata {formatDate(item.updated_at)}</p><div className="mt-3 flex flex-wrap gap-2"><Button asChild size="sm" variant="outline"><Link href={`/commercial/site-proposals/${item.id}`}>Apri</Link></Button>{canManage("crm") ? <Button size="sm" disabled={busyId === item.id} onClick={() => void generate(item)}>Genera</Button> : null}{canArchive ? <Button size="sm" variant="outline" onClick={() => setArchiveTarget(item)}>Archivia</Button> : null}{canPermanentlyDelete ? <Button size="sm" variant="destructive" onClick={() => setDeleteTarget(item)}>Elimina</Button> : null}</div></div></div></article>; })}</div>
