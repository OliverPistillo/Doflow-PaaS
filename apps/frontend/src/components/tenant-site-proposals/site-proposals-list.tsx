...
const PAGE_SIZE = 25;
function statusTone(status: string) { return status === "generated" ? "bg-emerald-100 text-emerald-700" : status === "error" ? "bg-rose-100 text-rose-700" : status === "ready" ? "bg-sky-100 text-sky-700" : "bg-muted text-muted-foreground"; }
...
export function SiteProposalsList() {
  ...
  const canArchive = canUseBuilder;
  ...
  return <main className="space-y-5 px-4 py-6 sm:px-6 lg:px-8">
    <CommercialPageHeader title="Proposte web" description="Crea, personalizza e genera proposte dimostrative mobile-first per i prospect." ctaLabel={canUseBuilder ? "Nuova proposta" : undefined} ctaHref={canUseBuilder ? "/commercial/site-proposals/new" : undefined} />
    ...
    <CommercialSectionCard title="Elenco proposte">
      ...
      {loading ? <div className="space-y-2">{Array.from({ length: 5 }).map((_, index) => <Skeleton key={index} className="h-16 w-full" />)}</div> : items.length === 0 ? <CommercialEmptyState>Nessuna proposta corrisponde ai filtri selezionati.</CommercialEmptyState> : <>
        <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[1080px] text-left text-sm"><thead className="border-b text-xs uppercase text-muted-foreground"><tr><th className="p-3"><Checkbox aria-label="Seleziona tutte le proposte della pagina" checked={allPageSelected ? true : somePageSelected ? "indeterminate" : false} onCheckedChange={(checked) => togglePage(checked === true)} /></th><th className="p-3">Proposta</th><th className="p-3">Template</th><th className="p-3">Stato e avanzamento</th><th className="p-3">Cliente CRM</th><th className="p-3">Progetto</th><th className="p-3">Versione</th><th className="p-3">Aggiornamento</th><th className="p-3" /></tr></thead><tbody>{items.map((item) => { const selected = selectedIds.has(item.id); const active = ["queued","running"].includes(item.preparationStatus || item.preparation_status || ""); return <tr key={item.id} className={`border-b last:border-0 ${selected ? "bg-primary/10" : ""}`}><td className="p-3"><Checkbox aria-label={`Seleziona ${item.display_name}`} checked={selected} onCheckedChange={(checked) => toggle(item.id, checked === true)} /></td><td className="p-3"><Link className="font-medium text-foreground hover:text-primary" href={`/commercial/site-proposals/${item.id}`}>{item.display_name}</Link><p className="mt-1 text-xs text-muted-foreground">{item.last_generated_at ? "Generata" : "Nessuna generazione"}</p></td><td className="p-3">{item.template_slug} <span className="text-xs text-muted-foreground">{item.template_version}</span></td><td className="p-3"><Badge className={statusTone(item.status)}>{proposalStatusLabel[item.status] || item.status}</Badge>{active ? <div className="mt-2"><SiteProposalProgress value={item} compact /></div> : null}</td><td className="p-3">{item.company_id ? "Collegata" : "—"}</td><td className="p-3">{item.project_id ? <Link className="font-medium text-primary hover:underline" href={`/dashboard/progetti/${item.project_id}`}>Apri progetto</Link> : "—"}</td><td className="p-3">v{item.current_version}</td><td className="p-3 text-muted-foreground">{formatDate(item.updated_at)}</td><td className="p-3"><div className="flex justify-end gap-1"><Button asChild size="icon" variant="ghost" aria-label={`Apri ${item.display_name}`}><Link href={`/commercial/site-proposals/${item.id}`}><Eye className="h-4 w-4" /></Link></Button>{canArchive ? <Button size="icon" variant="ghost" aria-label={`Archivia ${item.display_name}`} onClick={() => setArchiveTarget(item)}><Archive className="h-4 w-4" /></Button> : null}{canPermanentlyDelete ? <Button size="icon" variant="ghost" className="text-rose-600" aria-label={`Elimina definitivamente ${item.display_name}`} onClick={() => setDeleteTarget(item)}><Trash2 className="h-4 w-4" /></Button> : null}</div></td></tr>; })}</tbody></table></div>
        <div className="grid gap-3 md:hidden">{items.map((item) => { const selected = selectedIds.has(item.id); const active = ["queued","running"].includes(item.preparationStatus || item.preparation_status || ""); return <article key={item.id} className={`rounded-xl border p-4 ${selected ? "border-primary/40 bg-primary/10" : "border-border"}`}><div className="flex items-start gap-3"><Checkbox aria-label={`Seleziona ${item.display_name}`} checked={selected} onCheckedChange={(checked) => toggle(item.id, checked === true)} /><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><div><Link className="font-semibold text-foreground" href={`/commercial/site-proposals/${item.id}`}>{item.display_name}</Link><p className="mt-1 text-xs text-muted-foreground">{item.template_slug} · v{item.current_version}</p></div><Badge className={statusTone(item.status)}>{proposalStatusLabel[item.status] || item.status}</Badge></div>{active ? <div className="mt-3"><SiteProposalProgress value={item} compact /></div> : null}<p className="mt-3 text-xs text-muted-foreground">Aggiornata {formatDate(item.updated_at)}</p>{item.project_id ? <Link className="mt-2 inline-flex text-xs font-semibold text-primary hover:underline" href={`/dashboard/progetti/${item.project_id}`}>Apri progetto collegato</Link> : null}<div className="mt-3 flex flex-wrap gap-2"><Button asChild size="sm" variant="outline"><Link href={`/commercial/site-proposals/${item.id}`}>Apri</Link></Button>{canArchive ? <Button size="sm" variant="outline" onClick={() => setArchiveTarget(item)}>Archivia</Button> : null}{canPermanentlyDelete ? <Button size="sm" variant="destructive" onClick={() => setDeleteTarget(item)}>Elimina</Button> : null}</div></div></div></article>; })}</div>
        <div className="mt-4 flex items-center justify-between text-sm"><span className="text-muted-foreground">{total} risultati</span><div className="flex gap-2"><Button variant="outline" size="sm" disabled={offset === 0 || loading} onClick={() => { setSelectedIds(new Set()); setOffset(Math.max(0, offset - PAGE_SIZE)); }}>Precedente</Button><Button variant="outline" size="sm" disabled={offset + PAGE_SIZE >= total || loading} onClick={() => { setSelectedIds(new Set()); setOffset(offset + PAGE_SIZE); }}>Successiva</Button></div></div>
      </>}
