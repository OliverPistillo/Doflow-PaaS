"use client";

import { ShieldCheck } from "lucide-react";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";

export default function Page() {
  return (
    <div className="flex-1 p-4 md:p-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Sicurezza Avanzata</h2>
          <p className="text-sm text-muted-foreground mt-0.5">Audit log, ruoli granulari e IP whitelist</p>
        </div>
        <span className="text-xs font-semibold px-2 py-1 rounded-full bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300">Piano Enterprise</span>
      </div>

      <Card>
        <CardContent className="flex flex-col items-center justify-center py-20 text-center">
          <div className="h-16 w-16 rounded-2xl bg-amber-100 dark:bg-amber-900/20 flex items-center justify-center mb-4">
            <ShieldCheck className="h-8 w-8 text-amber-600 dark:text-amber-400" />
          </div>
          <h3 className="text-lg font-semibold mb-2">In arrivo</h3>
          <p className="text-sm text-muted-foreground max-w-sm">
            Questa sezione è in sviluppo. Sarà disponibile nelle prossime settimane.
          </p>
          <Button variant="outline" className="mt-6" size="sm">
            Torna alla Dashboard
          </Button>
        </CardContent>
      </Card>
    </div>
  );
}
