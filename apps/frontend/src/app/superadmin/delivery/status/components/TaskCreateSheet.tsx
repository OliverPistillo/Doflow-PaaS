...
// Fix: bg-primary/hover:bg-primary → bg-primary hover:bg-primary/90
...
type Task = {
  id: string; name: string; serviceName: string; category: string;
  dueDate: string; priority: string; notes: string; status: string;
};
...
export function TaskCreateSheet({ isOpen, onClose, onSuccess, taskToEdit }: TaskSheetProps) {
  ...
  const [formData, setFormData] = useState({
    name: "", serviceName: "", category: "Marketing",
    priority: "Media", dueDate: "", notes: "",
  });
    ...
    <Sheet open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
        <SheetHeader>
          ...
          <SheetDescription>
            {taskToEdit
              ? "Aggiorna i dettagli del task esistente."
              : "Aggiungi un nuovo task operativo al sistema."}
          </SheetDescription>
        </SheetHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-6">
          ...
          <div className="grid gap-2">
            ...
            <Input required value={formData.serviceName}
          ...
          <div className="grid gap-2">
            <Label>Note</Label>
            <Textarea className="resize-none h-32" placeholder="Dettagli aggiuntivi..."
