...
import { Server, HardDrive, Cpu, Activity, Database, Zap, Globe } from "lucide-react";
...
const StatusBadge = ({ label, status, icon: Icon }: any) => (
  <div
    className={`flex items-center justify-between p-4 rounded-xl border ${
      status === "up" ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"
    }`}
  >
...
export default function DashboardPage() {
  ...
  const loadData = async () => {
    try {
      // ✅ usa apiFetch: gestisce base URL + token + tenant header (quando serve)
  ...
  if (loading && !data) {
    return (
      <div className="p-10 flex items-center gap-2">
        <Activity className="animate-spin" /> Caricamento Control Tower...
      </div>
    );
  }

  if (errorMsg && !data) {
    return <div className="p-10 text-red-600 font-bold">{errorMsg}</div>;
  }
  ...
  return (
    <div className="space-y-8 max-w-[1600px] mx-auto animate-in fade-in">
      ...
      <div className="flex justify-between items-end">
        ...
        <div className="text-right">
          ...
          <div className="text-2xl font-mono font-black text-slate-700">
            {(hardware.uptime / 3600).toFixed(1)}h
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <StatusBadge
          label="Redis Cache"
          status={services.redis === "unknown" ? "up" : services.redis}
          icon={Zap}
        />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="p-6 rounded-2xl shadow-sm border-t-4 border-t-indigo-500">
          ...
          <Progress value={hardware.cpu.load} className="h-2 bg-slate-100" />
          <div className="text-xs text-slate-400 mt-4 font-medium">
            {hardware.cpu.cores} Cores - {hardware.cpu.brand}
          </div>
        </Card>
        ...
        <Card className="p-6 rounded-2xl shadow-sm border-t-4 border-t-emerald-500">
          ...
          <Progress value={hardware.memory.percent} className="h-2 bg-slate-100" />
        ...
        <Card className="p-6 rounded-2xl shadow-sm border-t-4 border-t-blue-500">
          ...
          <Progress value={hardware.disk.percent} className="h-2 bg-slate-100" />
          <div className="text-xs text-slate-400 mt-4 font-medium flex justify-between">
