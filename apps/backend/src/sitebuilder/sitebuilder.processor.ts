  ...
  private async generateGutenbergContent(
    ...
    for (const topic of payload.contentTopics) {
      const message = await this.anthropic.messages.create({
        ...
        max_tokens: 2048,
        system: `**Ruolo:**
Sei un Senior WordPress Developer e un esperto di serializzazione dati.

**Contesto:**
Devi generare il codice interno di una pagina WordPress. Il sistema di destinazione si aspetta di ricevere il contenuto tramite un payload JSON rigoroso per un'automazione API. Le variabili di sistema sono:
- Lingua del sito: ${payload.locale}
- Titolo del sito: ${payload.siteTitle}

**Obiettivo:**
Genera il markup HTML del contenuto utilizzando la sintassi dei blocchi Gutenberg e restituisci il risultato insieme al nome dell'argomento, inserendo il tutto all'interno di un oggetto JSON.

**Vincoli e Linee Guida:**
- OUTPUT ESCLUSIVO: La tua risposta deve essere unicamente l'oggetto JSON crudo.
- NESSUN FORMATO EXTRA: Non inserire blocchi markdown (niente \`\`\`json ... \`\`\`), niente backtick, nessun testo di benvenuto o spiegazione finale. Il primo carattere della risposta deve essere { e l'ultimo deve essere }.
- SINTASSI HTML: All'interno della stringa HTML, utilizza SOLO apici singoli per gli attributi (es. <div class='wp-block-group'>). Non usare MAI le doppie virgolette (") all'interno della stringa HTML, per non corrompere il parsing del JSON.
- BLOCCHI VALIDI: Utilizza solo commenti e blocchi Gutenberg standard validi (es. <!-- wp:paragraph -->, <!-- wp:heading -->, <!-- wp:group -->).

**Formato di Output:**
Restituisci ESCLUSIVAMENTE un oggetto JSON valido con questa struttura esatta:
{"topic": "<nome_argomento>", "html": "<codice_HTML_Gutenberg>"}`,
        messages: [{
          role: 'user',
      ...
      try {
        ...
        // Parse fallito: usa blocco placeholder Gutenberg valido e continua
        this.logger.warn(`[${payload.jobId}] Parse JSON fallito per topic "${topic}", uso fallback`);
        blocks.push({
          topic,
          html: `<!-- wp:heading --><h2 class='wp-block-heading'>${topic}</h2><!-- /wp:heading --><!-- wp:paragraph --><p>Contenuto della sezione ${topic}.</p><!-- /wp:paragraph -->`,
        });
      }
    }
  ...
  private async dockerComposeUp(deployDir: string, jobId: string): Promise<void> {
    ...
    // Tronca il log a 5 righe — il pull delle immagini è molto verboso
    const fullOutput = result.stdout || result.stderr;
    const shortOutput = fullOutput
      .split('\n')
      .filter((l) => l.trim())
      .slice(-5)
      .join(' | ');
    await this.appendLog(jobId, `docker-compose up: ${shortOutput}`);
