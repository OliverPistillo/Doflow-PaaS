...
describe('Tema Colsova 2.4.1 package', () => {
  ...
  it('keeps the doflow developer URL and contains no known prospect PII', () => { expect(config.business.developerUrl).toMatch(/^https:\/\/[^\s]*doflow/i); expect(html.toLowerCase()).not.toMatch(/federicanerone|elena\s*koltsova|345\s*463\s*2086/); });
});
