...
export class TenantSiteProposalsPreparationQueueService implements OnModuleInit {
    ...
    try {
      ...
      await runner.query(`
        INSERT INTO "${schema}".site_proposal_preparation_runs
          (id,proposal_id,job_id,status,force,reason,created_by,actor_email,job_data)
        VALUES ($1,$2,$1,'pending',$3,$4,$5,$6,$7::jsonb)
      `, [runId, proposalId, options.force, options.reason, data.actorUserId, data.actorEmail, JSON.stringify(data)]);
      await runner.query(`UPDATE "${schema}".site_proposals SET preparation_status='queued',preparation_error=NULL,preparation_queued_at=now(),latest_preparation_job_id=$1,updated_at=now() WHERE id=$2`, [runId, proposalId]);
  ...
  async markRunning(data: ProposalPreparationJobData) {
    this.jobData(data);
    await this.dataSource.query(`UPDATE "${SITE_PROPOSALS_TENANT}".site_proposal_preparation_runs SET status='running',started_at=COALESCE(started_at,now()),updated_at=now() WHERE id=$1 AND status IN ('dispatched','running')`, [data.preparationRunId]);
  }
  ...
  private error(error: unknown) { return cleanString(error instanceof Error ? error.message : String(error), 500) || 'Errore temporaneo di dispatch'; }
}
