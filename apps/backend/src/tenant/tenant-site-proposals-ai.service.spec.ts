...
const basicOutput = () => ({
  analysis: analysis(),
  content: { hero: { eyebrow: 'Studio', title: 'Hero', description: 'Descrizione hero', primaryCta: 'Contatti', secondaryCta: 'Servizi' }, approach: { title: 'Approccio', description: 'Descrizione approccio' }, services: rows(3, ['title', 'description']), benefits: { title: 'Benefici', description: 'Descrizione benefici', items: ['Primo beneficio', 'Secondo beneficio', 'Terzo beneficio'] }, trustItems: rows(6, ['title', 'description']), faq: rows(6, ['question', 'answer']), contact: { title: 'Contatti', description: 'Descrizione contatti', cta: 'Contattaci' }, footer: { text: 'Footer completo' } },
...
describe('Gemini proposal provider', () => {
  ...
  it.each(['hero','approach','services','benefits','trustItems','faq','contact','footer'])('rejects an independently empty basic section: %s', (section) => { const value: any = basicOutput(); value.content[section] = Array.isArray(value.content[section]) ? [] : {}; expect(() => service.validate(value, {}, 'proposal-basic-v2')).toThrow(ProposalAiUnavailableError); });
  it('sends nested required/minLength rules for every basic section', () => { const schema: any = (service as any).schema('proposal-basic-v2'); const content = schema.properties.content; for (const key of ['hero','approach','services','benefits','trustItems','faq','contact','footer']) expect(content.properties[key]).toEqual(expect.objectContaining({ type: expect.any(String) })); expect(content.properties.hero.required).toEqual(expect.arrayContaining(['eyebrow','title','description','primaryCta','secondaryCta'])); expect(content.properties.footer.properties.text.minLength).toBe(1); });
