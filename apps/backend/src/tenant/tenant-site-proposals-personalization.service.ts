...
export class TenantSiteProposalsPersonalizationService {
  ...
  private assertAccess():AuthUserRef{const user=this.request?.user;if(!user||!hasRoleAtLeast(String(user.role||''),'manager'))throw new ForbiddenException('Accesso non autorizzato');return {id:String(user.id||''),email:user.email||null,role:String(user.role)};}
  ...
  private schema(){const schema=safeSchema(String(this.request?.tenant?.schemaName||this.request?.tenant?.schema||''),'site proposals personalization');if(schema!==SITE_PROPOSALS_TENANT)throw new ForbiddenException('Funzione disponibile solo per doflow');return schema;}
