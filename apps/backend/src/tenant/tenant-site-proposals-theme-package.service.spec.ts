...
function basicPackage(root = '') {
  ...
  const html = `<!doctype html><meta name="robots" content="noindex,nofollow,noarchive"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; connect-src 'none'; object-src 'none'; frame-src 'none'; form-action 'none'; img-src data:; style-src 'unsafe-inline'; script-src 'unsafe-inline'"><script id="template-config" type="application/json">${JSON.stringify(config)}</script>`;
...
describe('theme ZIP package validator', () => {
  ...
  it('contains explicit defenses for traversal, symlinks, duplicates, bombs and file count', () => { const source = fs.readFileSync(path.join(__dirname, 'tenant-site-proposals-theme-package.service.ts'), 'utf8'); for (const marker of ["part === '..'", 'unixType === 0xa000', 'seen.has(lower)', 'MAX_UNCOMPRESSED', 'MAX_FILES']) expect(source).toContain(marker); });
});
