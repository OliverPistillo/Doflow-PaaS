...
async function main() {
  ...
  if (!['localhost', '127.0.0.1', 'doflow-acceptance-postgres'].includes(host) || !/acceptance/i.test(database)) {
    throw new Error('Collaboration acceptance migration requires an isolated acceptance database');
  }
  ...
  try {
    ...
    await new CreateCollaborationNotificationsRealtimeAuthority1830000000000().up(runner);
  } finally {
